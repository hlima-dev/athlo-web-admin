...
export function Donations() {
  ...
  const donors = new Set(donations.map((d) => d.donorEmail || d.donorName).filter(Boolean)).size;
  ...
  const fmtMethod: Record<string, string> = {
    PIX: "PIX", CREDIT_CARD: "Cartao credito", BANK_TRANSFER: "Transferencia",
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-4">
        {[
          { icon: Wallet, color: "text-cyan-400", label: "Total arrecadado", value: loading ? "..." : fmt(totalRaised) },
          { icon: TrendingUp, color: "text-emerald-400", label: "Doacoes confirmadas", value: loading ? "..." : confirmed.length },
          { icon: Users, color: "text-purple-400", label: "Doadores", value: loading ? "..." : donors },
          { icon: CalendarDays, color: "text-yellow-400", label: "Registros", value: loading ? "..." : donations.length },
        ].map((item) => {
            ...
            donations.map((d) => (
              <div key={d.id} className="flex flex-col gap-4 rounded-2xl bg-slate-800 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-black">{d.donorName || "Doador nao informado"}</h3>
                    <p className="text-sm text-slate-400">
                      {fmtMethod[d.method] || d.method} - {fmtDate(d.createdAt)}
                    </p>
                  </div>
                </div>
                <strong className="text-xl text-cyan-400">{fmt(d.amount)}</strong>
              </div>
            ))
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <form onSubmit={handleCreate} className="w-full max-w-2xl rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <input type="number" min="1" step="0.01" placeholder="Valor"
